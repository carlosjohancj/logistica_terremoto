      ...
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <Link
            ...
            className={cn(
              "text-sm font-medium transition-colors hover:text-primary",
              isActive(`/${locale}/solicitar-viaje`) ? "text-primary" : "text-muted-foreground"
            )}
          >
            {t("solicitarViaje")}
          </Link>
          ...
          >
            <DropdownLink href={`/${locale}/ofrecer-transporte`} active={isActive(`/${locale}/ofrecer-transporte`)}>
              {t("ofrecerTransporte")}
            </DropdownLink>
            <DropdownLink href={`/${locale}/ofrecer-hospedaje`} active={isActive(`/${locale}/ofrecer-hospedaje`)}>
              {t("ofrecerHospedaje")}
            </DropdownLink>
            <DropdownLink href={`/${locale}/donaciones-fisicas`} active={isActive(`/${locale}/donaciones-fisicas`)}>
              {t("donacionesFisicas")}
            </DropdownLink>
          </NavDropdown>
          ...
          <Link
            ...
            className={cn(
              "text-sm font-medium transition-colors hover:text-primary",
              isActive(`/${locale}/explorar`) ? "text-primary" : "text-muted-foreground"
            )}
          >
            {t("explorar")}
          </Link>
          ...
          <Link
            ...
            className={cn(
              "text-sm font-medium transition-colors hover:text-primary",
              isActive(`/${locale}/donar`) ? "text-primary" : "text-muted-foreground"
            )}
          >
            {t("donar")}
          </Link>
          ...
          <NavDropdown label="Más" active={masPaths.some(isActive)}>
            <DropdownLink href={`/${locale}/empleos`} active={isActive(`/${locale}/empleos`)}>
              {t("empleos")}
            </DropdownLink>
            <DropdownLink href={`/${locale}/recursos`} active={isActive(`/${locale}/recursos`)}>
              {t("recursos")}
            </DropdownLink>
            <DropdownLink href={`/${locale}/sobre-nosotros`} active={isActive(`/${locale}/sobre-nosotros`)}>
              {t("sobreNosotros")}
            </DropdownLink>
          </NavDropdown>
        </nav>
            ...
            {isLoggedIn ? (
              <>
                <Link href={`/${locale}/matches`}>
                  <Button variant="ghost" size="sm">{t("matches")}</Button>
                </Link>
