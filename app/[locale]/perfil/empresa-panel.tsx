...
export default function EmpresaPanel() {
  ...
  async function closeJob(jobId: string) {
    try {
      const supabase = getSupabase()
      await supabase.from(TABLES.JOBS).update({ status: "closed" as never }).eq("id", jobId)
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-semibold">{tj("title")} - Panel</h2>
          <p className="text-sm text-muted-foreground">{tj("subtitle")}</p>
        </div>
        ...
        {jobs.map((job) => (
          <Card key={job.id}>
            <CardContent className="flex items-center justify-between py-4">
              ...
              {job.status === "open" && (
                <Button variant="ghost" size="sm" onClick={() => closeJob(job.id)} className="shrink-0 ml-4">
